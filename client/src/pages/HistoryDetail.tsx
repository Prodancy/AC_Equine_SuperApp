  ...
  return (
    <div className="p-4 md:p-8 max-w-5xl mx-auto space-y-6 pb-24 md:pb-8">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl md:text-3xl font-bold tracking-tight text-foreground uppercase tracking-widest">Session Detail</h1>
      ...
      <div className="grid gap-6 md:grid-cols-3">
        <div className="md:col-span-2 space-y-6">
          ...
          <Card className="bg-card/50 backdrop-blur border-white/5 overflow-hidden shadow-xl">
            <CardHeader className="pb-4">
              <CardTitle className="text-xs font-black uppercase tracking-[0.2em] text-primary">Thermal Baseline & Post-Op</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-2 gap-4">
                {detail.images.map((img, i) => (
                  <div key={i} className="relative aspect-square rounded-2xl overflow-hidden border border-white/10 bg-black/50 group shadow-2xl">
                    ...
                    />
                    <div className="absolute top-3 left-3 bg-black/80 text-white text-[9px] px-2 py-1 rounded-md backdrop-blur-md font-black border border-white/10 tracking-widest uppercase">
          ...
          <Card className="bg-card/50 backdrop-blur border-white/5 shadow-xl">
            <CardHeader className="pb-2">
              <CardTitle className="text-xs font-black uppercase tracking-[0.2em] text-primary">Clinical Assessment</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="bg-primary/5 p-4 rounded-xl border border-primary/10">
                <div className="flex items-center gap-2 mb-2 text-primary">
                  <Stethoscope className="w-4 h-4" />
                  <span className="text-xs font-bold uppercase tracking-wider">{detail.category}</span>
                </div>
                <p className="text-sm text-gray-300 leading-relaxed italic">"{detail.notes}"</p>
                <div className="mt-4 pt-4 border-t border-primary/10 text-[10px] text-muted-foreground font-bold uppercase tracking-widest">
        ...
        <div className="space-y-6">
          ...
          <Card className="bg-card/50 backdrop-blur border-white/5 shadow-xl border-t-4 border-t-primary">
            <CardHeader>
              <CardTitle className="text-xs font-black uppercase tracking-[0.2em] text-muted-foreground">Session Metrics</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="flex justify-between items-center">
                <span className="text-xs text-muted-foreground uppercase font-bold tracking-wider">Status</span>
                <Badge className="bg-green-500/20 text-green-500 border-green-500/30 font-black uppercase text-[10px] tracking-tighter">
                  {detail.status}
                </Badge>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-xs text-muted-foreground uppercase font-bold tracking-wider">Protocol</span>
                <span className="text-sm font-bold text-white tracking-tight">{detail.protocol}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-xs text-muted-foreground uppercase font-bold tracking-wider">Target Temp</span>
          ...
          <Card className="bg-card/50 backdrop-blur border-white/5 shadow-xl">
            <CardHeader>
              <CardTitle className="text-xs font-black uppercase tracking-[0.2em] text-muted-foreground">Treatment Modalities</CardTitle>
            </CardHeader>
            <CardContent className="grid grid-cols-1 gap-2">
              {detail.proposedTreatments.map((t, i) => (
                <div key={i} className="flex items-center gap-3 bg-white/5 p-3 rounded-xl border border-white/5">
                  <t.icon className={cn("w-4 h-4", t.color)} />
                  <span className="text-[10px] font-black uppercase tracking-widest text-gray-300">{t.label}</span>
                </div>
              ))}
            </CardContent>
          </Card>
          ...
          <Card className="bg-orange-500/5 backdrop-blur border-orange-500/20 shadow-xl">
            <CardHeader>
              <CardTitle className="text-xs font-black uppercase tracking-[0.2em] text-orange-400 flex items-center gap-2">
            ...
            <CardContent className="space-y-3">
              {detail.missedTreatments.map((m, i) => (
                <div key={i} className="text-xs p-3 rounded-xl bg-orange-500/10 border border-orange-500/20 space-y-1">
                  <div className="flex justify-between font-bold text-orange-200 uppercase tracking-tighter">
                    <span>{m.date}</span>
