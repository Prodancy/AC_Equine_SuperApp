...
import { Play, Pause, Square, Zap, Bluetooth, Activity, ChevronLeft } from "lucide-react";
import { useToast } from "@/hooks/use-toast";
import { cn } from "@/lib/utils";
import { Link } from "wouter";

const protocols = [
  { id: "l1", name: "Acute Pain", duration: 8, intensity: 50, frequency: "1000Hz" },
  { id: "l2", name: "Deep Tissue Healing", duration: 12, intensity: 70, frequency: "2500Hz" },
  { id: "l3", name: "Superficial Wound", duration: 5, intensity: 30, frequency: "Continuous" },
];

export default function Laser3B() {
    ...
    if (isPlaying) {
      interval = setInterval(() => {
        setTimeLeft((prev) => {
          if (prev <= 1) {
            setIsPlaying(false);
            toast({ title: "3B Laser Complete", description: "The session has finished successfully." });
            return 0;
          }
          return prev - 1;
        });
    ...
    <div className="p-4 md:p-8 max-w-5xl mx-auto space-y-6 pb-24 md:pb-8">
      ...
      <div className="flex justify-between items-center bg-card/30 backdrop-blur-md sticky top-0 z-50 py-4 px-4 md:px-8 border-b border-white/5 -mx-4 md:-mx-8 mb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl md:text-2xl font-bold tracking-widest text-foreground">3B Laser Therapy</h1>
            <p className="text-[10px] text-muted-foreground tracking-tight hidden md:block">Active Laser Session</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <div className="space-y-6 order-1 md:order-2">
          <Card className="flex flex-col justify-between overflow-hidden relative border-red-400/20 shadow-lg bg-card">
            <CardHeader className="text-center pb-0 pt-4">
              <CardTitle className="text-muted-foreground tracking-widest text-[10px] md:text-xs">Laser Exposure</CardTitle>
            </CardHeader>
            <CardContent className="flex-1 flex flex-col items-center justify-center space-y-6 md:space-y-8 relative z-10 py-6 md:py-8">
              <div className="relative w-56 h-56 md:w-64 md:h-64 flex items-center justify-center">
                <svg className="w-full h-full transform -rotate-90">
                  ...
                  <circle cx="50%" cy="50%" r="45%" stroke="currentColor" strokeWidth="8" fill="transparent" strokeDasharray="283" strokeDashoffset={283 * (1 - progress / 100)} className="text-red-400 transition-all duration-1000 ease-linear" />
                </svg>
              ...
              <div className="grid grid-cols-2 gap-4 md:gap-8 w-full">
                <div className="flex flex-col items-center bg-red-400/5 p-3 md:p-4 rounded-xl border border-red-400/10">
                  <Activity className="w-5 h-5 md:w-6 md:h-6 text-red-400 mb-1" />
                  <span className="text-xl md:text-2xl font-bold tabular-nums">{currentProtocol.frequency}</span>
                  <span className="text-[10px] md:text-xs text-muted-foreground">Frequency</span>
                </div>
                <div className="flex flex-col items-center bg-red-400/5 p-3 md:p-4 rounded-xl border border-red-400/10">
                  <Zap className="w-5 h-5 md:w-6 md:h-6 text-red-400 mb-1" />
        ...
        <div className="space-y-4 md:space-y-6 order-2 md:order-1">
          <Card>
            <CardHeader className="pb-2 md:pb-4">
              <CardTitle className="text-base md:text-lg">Laser Protocol</CardTitle>
            </CardHeader>
              ...
              <div className="pt-2 space-y-4">
                <div className="space-y-3">
                  <div className="flex justify-between text-sm font-medium">
                    ...
                    <span className="text-red-400 font-bold">{intensity[0]}%</span>
                  </div>
          ...
          <div className="grid grid-cols-2 gap-3 md:gap-4">
            <Button size="lg" className={cn("h-20 text-xl rounded-2xl", isPlaying ? "bg-yellow-500 hover:bg-yellow-600" : "bg-red-500 hover:bg-red-600")} onClick={() => setIsPlaying(!isPlaying)}>
