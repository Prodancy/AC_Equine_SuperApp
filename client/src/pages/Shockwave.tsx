...
import { Play, Pause, Square, Waves, Bluetooth, Activity, ChevronLeft } from "lucide-react";
import { useToast } from "@/hooks/use-toast";
import { cn } from "@/lib/utils";
import { Link } from "wouter";

const protocols = [
  { id: "s1", name: "Tendonitis", duration: 12, intensity: 60, shocks: 2000 },
  { id: "s2", name: "Calcification", duration: 15, intensity: 80, shocks: 3000 },
  { id: "s3", name: "Muscle Trigger", duration: 10, intensity: 40, shocks: 1500 },
];
    ...
    if (isPlaying) {
      interval = setInterval(() => {
        setTimeLeft((prev) => {
          if (prev <= 1) {
            setIsPlaying(false);
            toast({ title: "Shockwave Complete", description: "The session has finished successfully." });
            return 0;
          }
          return prev - 1;
        });
    ...
    <div className="p-4 md:p-8 max-w-5xl mx-auto space-y-6 pb-24 md:pb-8">
      ...
      <div className="flex justify-between items-center bg-card/30 backdrop-blur-md sticky top-0 z-50 py-4 px-4 md:px-8 border-b border-white/5 -mx-4 md:-mx-8 mb-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-xl md:text-2xl font-bold tracking-widest text-foreground">Shockwave Therapy</h1>
            <p className="text-[10px] text-muted-foreground tracking-tight hidden md:block">Active ESWT Session</p>
          </div>
        </div>
      </div>
        ...
        <div className="space-y-4 md:space-y-6 order-2 md:order-1">
          <Card>
            <CardHeader className="pb-2 md:pb-4">
              <CardTitle className="text-base md:text-lg">Shockwave Protocol</CardTitle>
            </CardHeader>
