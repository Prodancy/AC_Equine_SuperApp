    ...
    <div className="p-4 md:p-8 max-w-5xl mx-auto space-y-6 pb-24 md:pb-8">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        <div className="space-y-6 order-1 md:order-2">
          <Card className="flex flex-col justify-between overflow-hidden relative border-purple-400/20 shadow-lg bg-card">
            <CardHeader className="text-center pb-0 pt-4">
              <CardTitle className="text-muted-foreground tracking-widest text-[10px] md:text-xs">Power Output</CardTitle>
            </CardHeader>
            <CardContent className="flex-1 flex flex-col items-center justify-center space-y-6 md:space-y-8 relative z-10 py-6 md:py-8">
              <div className="relative w-56 h-56 md:w-64 md:h-64 flex items-center justify-center">
                <svg className="w-full h-full transform -rotate-90">
                  ...
                  <circle cx="50%" cy="50%" r="45%" stroke="currentColor" strokeWidth="8" fill="transparent" strokeDasharray="283" strokeDashoffset={283 * (1 - progress / 100)} className="text-purple-400 transition-all duration-1000 ease-linear" />
                </svg>
              ...
              <div className="grid grid-cols-2 gap-4 md:gap-8 w-full">
                <div className="flex flex-col items-center bg-purple-400/5 p-3 md:p-4 rounded-xl border border-purple-400/10">
                  <Zap className="w-5 h-5 md:w-6 md:h-6 text-purple-400 mb-1" />
                  <span className="text-xl md:text-2xl font-bold tabular-nums">{currentProtocol.power}</span>
                  <span className="text-[10px] md:text-xs text-muted-foreground">Power Level</span>
                </div>
                <div className="flex flex-col items-center bg-purple-400/5 p-3 md:p-4 rounded-xl border border-purple-400/10">
                  <ShieldCheck className="w-5 h-5 md:w-6 md:h-6 text-purple-400 mb-1" />
                  <span className="text-xl md:text-2xl font-bold tabular-nums">{intensity[0]}%</span>
                  <span className="text-[10px] md:text-xs text-muted-foreground">Duty Cycle</span>
                </div>
              </div>
            </CardContent>
          </Card>
        ...
        <div className="space-y-4 md:space-y-6 order-2 md:order-1">
              ...
              <div className="pt-2 space-y-4">
                <div className="space-y-3">
                  <div className="flex justify-between text-sm font-medium">
                    ...
                    <span className="text-purple-400 font-bold">{intensity[0]}%</span>
                  </div>
          ...
          <div className="grid grid-cols-2 gap-3 md:gap-4">
            <Button size="lg" className={cn("h-20 text-xl rounded-2xl", isPlaying ? "bg-yellow-500 hover:bg-yellow-600" : "bg-purple-500 hover:bg-purple-600")} onClick={() => setIsPlaying(!isPlaying)}>
